...
import copy as cp
from sklearn.metrics import f1_score, accuracy_score, recall_score, roc_auc_score, average_precision_score, classification_report
...
def convert_data_to_sparse(prefix):
    dict = {}
    edges = pd.read_csv(os.path.join(prefix, 'graph', 'social_network_test.edg'), sep='\t', header=None)
    row = np.array(edges[0])
    col = np.array(edges[1])
    print(len(row), len(col))
    data = np.ones(shape=len(row), dtype=np.float64)
    edges_csc = csc_matrix((data, (row, col)))
    dict['net_ufu'] = edges_csc # user follows user


def load_data(data, prefix, w2v_size):
    """
    ...
    #prefix = '/home/ghizzotae/big-data/RioGNN/data/Twitter/piccolo'
    if data == 'yelp':
        ...
        data_file = deserialize(os.path.join(prefix, 'dataset-'+str(w2v_size)+'.dict'))
        labels = data_file['label'].flatten()
        feat_data = data_file['features'].todense().A
        # load the preprocessed adj_lists
        with open(os.path.join(prefix, 'net_ufu_adjlists.pickle'), 'rb') as file:
            relation1 = pickle.load(file)
        with open(os.path.join(prefix, 'net_spatial_adjlists.pickle'), 'rb') as file:
            relation2= pickle.load(file)
...
def test_rio(test_cases, labels, model, batch_size):
    ...
    for iteration in range(test_batch_num):
        ...
        f1_gnn += f1_score(batch_label, gnn_prob.data.cpu().numpy().argmax(axis=1), average="macro", zero_division=0.0)
        acc_gnn += accuracy_score(batch_label, gnn_prob.data.cpu().numpy().argmax(axis=1))
        recall_gnn += recall_score(batch_label, gnn_prob.data.cpu().numpy().argmax(axis=1), average="macro", zero_division=0.0)

        f1_label1 += f1_score(batch_label, label_prob1.data.cpu().numpy().argmax(axis=1), average="macro", zero_division=0.0)
        acc_label1 += accuracy_score(batch_label, label_prob1.data.cpu().numpy().argmax(axis=1))
        recall_label1 += recall_score(batch_label, label_prob1.data.cpu().numpy().argmax(axis=1), average="macro", zero_division=0.0)

        gnn_list.extend(gnn_prob.data.cpu().numpy()[:, 1].tolist())
        label_list1_argmax.extend(label_prob1.data.cpu().numpy().argmax(axis=1))
    ...
    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~', flush=True)
    print(classification_report(labels, label_list1_argmax, zero_division=0.0))
    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~', flush=True)
...
def test_rio_final(test_cases, labels, model, batch_size, log_save_path, epoch, loss):
    """
    Test the performance of Rio-GNN and its variants
    :param test_cases: a list of testing node
    :param labels: a list of testing node labels
    ...
    with open(log_save_path + '/test_performance.txt', 'a') as file:
        file.write('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n')
        file.write('EPOCH: {}\n'.format(epoch))
        file.write('Loss: {}\n'.format(loss))
        file.write('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n')
        ...
        file.write('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n')
        file.writelines(classification_report(labels, label_list1_argmax, zero_division=0.0))
        file.write('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n')
        file.write(f"TEST GNN F1: {f1_gnn / test_batch_num:.4f}\n")
        file.write(f"TEST GNN Accuracy: {acc_gnn / test_batch_num:.4f}\n")
        file.write(f"TEST GNN Recall: {recall_gnn / test_batch_num:.4f}\n")
        file.write(f"TEST GNN auc: {auc_gnn:.4f}\n")
        file.write(f"TEST GNN ap: {ap_gnn:.4f}\n")
        file.write('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n')
        file.write(f"TEST Label1 F1: {f1_label1 / test_batch_num:.4f}\n")
        file.write(f"TEST Label1 Accuracy: {acc_label1 / test_batch_num:.4f}\n")
        file.write(f"TEST Label1 Recall: {recall_label1 / test_batch_num:.4f}\n")
        file.write(f"TEST Label1 auc: {auc_label1:.4f}\n")
        file.write(f"TEST Label1 ap: {ap_label1:.4f}\n")
        file.write('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n')
        file.write('\n')
        file.write('\n')
