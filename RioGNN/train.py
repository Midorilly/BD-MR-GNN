...
parser.add_argument('--w2v_size', type=int, required=True, help='Embedding size of W2V features')
parser.add_argument('--log_path', default='/home/ghizzotae/big-data/RioGNN/log/merged/128', type=str, help="Path of results")
...
# hyper-parameters
parser.add_argument('--lr', type=float, default=0.02, help='Initial learning rate.') # eventualmente abbassare
...
parser.add_argument('--test_epochs', type=int, default=1, help='Epoch interval to run test set.')
parser.add_argument('--test_ratio', type=float, default=0.60, help='Test set size.')
parser.add_argument('--under_sample', type=int, default=1.0, help='Under-sampling scale.')
parser.add_argument('--classes_weight', type=float, default=1.0, help='Classes weight multiplier.')
...
if __name__ == '__main__':
    ...
    # generate log folder
    log_save_path = os.path.join(args.log_path, 'log_' + str(args.w2v_size) + '_' +str(args.lr) + '_' + str(args.classes_weight) + '_' + str(args.num_epochs) + '_' + strftime("%m%d%H%M%S", localtime()))
    ...
    homo, relations, feat_data, labels, index = load_data(args.data, args.data_path, args.w2v_size)
    ...
    twitter = deserialize(os.path.join(args.data_path, 'dataset-'+str(args.w2v_size)+'.dict'))
    idx_train = index[:len(twitter['train_indexes'])]
    y_train = labels[:len(twitter['train_labels'])]
    idx_test = index[len(twitter['train_indexes']):]
    y_test = labels[len(twitter['train_labels']):]

    nz = len([x for x in y_train if x == 1])
    pos_weight = len(y_train) / nz
    neg_weight = len(y_train) / (args.classes_weight*(len(y_train) - nz))

    #pos_weight = twitter['pos_weight']
    #neg_weight = twitter['neg_weight']

    #idx_train, idx_test, y_train, y_test = train_test_split(index, labels, stratify=labels, test_size=args.test_ratio, random_state=2, shuffle=True)
    ...
    if args.model == 'RIO':
        ...
        inter1 = InterAgg(width_rl, height_rl, args.device, args.LR, args.GAMMA, args.stop_num,
                          ...
                          cuda=args.cuda)
        gnn_model = OneLayerRio(2, inter1, args.lambda_1, class_weights)
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in range(num_batches):
            ...
            optimizer.zero_grad()
            if args.cuda:
                loss = gnn_model.loss(batch_nodes, Variable(torch.LongTensor(batch_label, device='cuda:7')))
            else:
    ...
    gnn_auc, label_auc, gnn_recall, label_recall = test_rio_final(idx_test, y_test, gnn_model, args.batch_size, log_save_path, epoch, loss.item() / num_batches)
